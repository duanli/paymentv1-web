...
<html>
	...
	<body>
		<div class="place">
		    ...
		    <ul class="placeul">
			    <li><a href="#">首页</a></li>
			    <li><a href="${ctx}/providerStatisticsDays/toProviderStatisticsDaysPage">ProviderStatisticsDays</a></li>
		    </ul>
		</div>
		<!-- buttons -->
		<div class="rightinfo">
			<div style="margin:1px 5px 5px 1px;float: left;">
				...
				<li>交易通道:<input name="productId" type="text" maxlength="30" id="productId_s" class="easyui-textbox" style="width:100px;" value=""/></li>
				<li>交易账户:<input name="providerAccId" type="text" maxlength="30" id="providerAccId_s" class="easyui-textbox" style="width:100px;" value=""/></li>
				...
				<li>统计月份:<input name="sdMouth" type="text" maxlength="30" id="sdMouth_s" class="easyui-textbox" style="width:100px;" value=""/></li>
			 	...
			 	<li class="click" id="toStorePage" onclick="javascript:search();" style="border:solid 1px #d3dbde;">
			 		...
			 		<shiro:hasPermission name="providerStatisticsDays:add">
			 		     <li class="click" id="addProviderStatisticsDays" onclick="javascript:addProviderStatisticsDaysDialog();">
				     		<span><a href="#"><img src="${ctx}/images/t01.png"/></a></span>新增
			     		</li>  
					</shiro:hasPermission>   
					<shiro:hasPermission name="providerStatisticsDays:update">
					 	<li class="click" id="updateProviderStatisticsDays" onclick="javascript:updateProviderStatisticsDaysDialog();">
			 	 			<span><a href="#"><img src="${ctx}/images/t02.png"/></a></span>编辑
		 	 			</li>
					</shiro:hasPermission>
					<shiro:hasPermission name="providerStatisticsDays:delete">
						<li class="click" id="deleteProviderStatisticsDays"  onclick="javascript:deleteProviderStatisticsDays();">
		...
		<!-- dialog -->
 		<div id="providerStatisticsDaysDialog" title="操作菜单" class="easyui-dialog" closed="true"  style="width:830px;height:560px;padding:10px" data-options="iconCls:'icon-save',modal:true" buttons="#dialog_buttons">
			<div class="tab">
 				<form class="easyui-form" id="providerStatisticsDaysForm" method="post">
					<fieldset style="width:800px;height:530px">
 						...
 						<table border="0">
									...
									<tr>
                					 <td>服务商交易方式Id<b>*</b></td><td><input id="productId" name="productId" type="text" class="easyui-validatebox" data-options="required:true,missingMessage:'服务商交易方式Id不能为空!'"></input></td>
									</tr>
									<tr>
                					 <td>支付账户主键<b>*</b></td><td><input id="providerAccId" name="providerAccId" type="text" class="easyui-validatebox" data-options="required:true,missingMessage:'支付账户主键不能为空!'"></input></td>
									</tr>
									<tr>
		...
		<div id="dialog_buttons">
			...
			<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="$('#providerStatisticsDaysDialog').dialog('close');" style="width:70px;height:30px;">关闭</a>
		</div>
		<script type="text/javascript">
			...
			function initGrid(){
				 $('#tt').datagrid({
						url:'${ctx}/providerStatisticsDays/getProviderStatisticsDaysPage',
						pagination:true,
						rownumbers : true,
						fitColumns : true,
						collapsible : true,
			...
			function regCRUDEvent(){
				...
				$('#submitFormBtn').click(function(e) {
					options.url = '${ctx}/providerStatisticsDays/'+url;
					$('#providerStatisticsDaysForm').ajaxSubmit(options);
			        return false;
				});
				// 提交参数对象
			  	var options = {
				...
				// 表单检查
			  	function before(){
					if($('#providerStatisticsDaysForm').form('validate')){
						pro();
						$('#providerStatisticsDaysDialog').dialog('close');
						return true;
					}else
						return false;
			  	};
		        ...
		        function showError(p1, $form) {
		        	hide();
		        	$('#providerStatisticsDaysDialog').dialog('open');
		        	$.messager.alert('提示',eval(p1.responseText), 'info');
		        };
			}
			
			function addProviderStatisticsDaysDialog(){
				$('#providerStatisticsDaysForm').form('clear');
				$('#providerStatisticsDaysDialog').dialog('open');
				url = "addProviderStatisticsDays";
			}
			// response,status,xhr
		    function updateProviderStatisticsDaysDialog(){
		    	url = "updateProviderStatisticsDays";
		    	var chks= $('#tt').datagrid('getChecked');
	    		if(null!=chks && chks.length==1){
	    			$.ajaxSettings.async = true;
	    			pro();
	    			var id = chks[0].sdId;
	    		 	$.ajax({type:'GET',url:'${ctx}/providerStatisticsDays/getProviderStatisticsDaysById',data:{'id':id,'r':Math.random()},
		    ...
		    function deleteProviderStatisticsDays(){
		    	var chks = null;
	        	...
	        	$.messager.confirm('提示', '确定要删除该项目吗?', function(yes){
	        		if(yes){
	        			pro();
	        			$.ajax({type:'GET',url:'${ctx}/providerStatisticsDays/deleteProviderStatisticsDays',data:{'param':arr.toString()},
